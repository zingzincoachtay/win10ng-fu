...
module.exports.makeXLSX = (filename,_o,_h) => {
  console.log("Attempt to write: "+filename+'.xlsx');
...
module.exports.makeDumpImportable = (data,source,bp, rows=[] ) => {// source is `new Map()` object with a relational data
  ...
  data.forEach((datum, i) => {
    ...
    let freeze = fileProperty.slice(-1).concat(// Will always keep the full file path at the end of Array object
                    ...flattenOOO([],bp.Referrer["basic.para"],datum[0].basic.para),
                    ...flattenOOO([],bp.Referrer["basic.prof"],datum[0].basic.prof)
                 );
    for(let dbKey of bp.dbKeyDefCol.slice(2))
        rows.push( ...flattenOOO(freeze.concat(dbKey),bp.Referrer[dbKey], traverse(dbKey.split(/\W+/g),datum[0])) );// `traverse` points to an object
  });
  return rows;
};
